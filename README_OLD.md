...
### Smart Query Execution (v0.0.8+)

The library now includes a `QueryExecutor` that automatically handles both direct and correlation queries:
...
// Direct query - automatically detected and routed
const directQuery = "graylog-prod(level:error)[5m]";
for await (const event of executor.execute(directQuery)) {
  console.log("Log event:", event);
}

// Correlation query - automatically detected and handled
